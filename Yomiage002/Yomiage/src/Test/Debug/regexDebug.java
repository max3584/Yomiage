...
public class regexDebug {
	...
	public static void main(String[] args) {
		String text = "/la hello";
		String text2 = "/cla fun";
		// String regex =
		// "(^|\\s)/[a-zA-Z]*;(^|\\s)/la\\s\\w?(\\ss+\\d.\\d)?;(^|\\s)/ci\\d+?(\\s(\\d+|nw|t\\d|s\\d+)){1,4};(^|\\s)/cmf?\\b\\w?";
		// /la ichitaro;

		// la
		String regex1 = "(^|\\s)/(f|m|c)*la\\s+\\w*";
		// sr
		...
		System.out.println(String.format("submit: %s",text.replaceAll(regex1, "").replaceAll(regex2, "").replaceAll(regex3, "")
		...
		System.out.println(String.format("submit: %s",text2.replaceAll(regex1, "").replaceAll(regex3, "").replaceAll(regex4, "")
